...
主进程使用`BrowserWindow实例`创建页面。每个`BrowserWindow实例`都在自己的渲染进程里运行页面。当一个`BrowserWindow实例`被销毁后，相应的渲染进程也会被终止。
...
有了上述概念性的理解，接下来编写整个应用的入口文件`main.js`。在根目录下创建一个`main.js`，在`package.json`中加入以下配置。
...
有一个地方需注意。在开发阶段，页面以浏览器内存的形式存在，简单来说就是一个可访问的地址。在测试阶段和生产阶段，页面以实体文件的形式存在，简单来说就是`webpack`打包输出的文件根目录路径。所以在这两种情况，`BrowserWindow实例`需在不同情况使用不同的方法引用页面。
...
- **开发模式**：页面的在线调试地址为`http://127.0.0.1:9999`，端口在启动构建进程时可自行选择
- **测试模式**：页面的文件打包路径为`dist/test`，HTML入口为`dist/test/index.html`
- **生产模式**：页面的文件打包路径为`dist/prod`，HTML入口为`dist/prod/index.html`

在开发阶段可为网页的渲染进程开启一个`Chrome DevTools`，方便后续的开发调试。
